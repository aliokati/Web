...
export type OptionalKeys<T> = Values<{
    [K in keyof T]: unknown extends T[K] ? never : OmitProperty extends T[K] ? K : never;
}>;
export type RequiredKeys<T> = Values<{
    [K in keyof T]: unknown extends T[K] ? K : OmitProperty extends T[K] ? never : K;
}>;
/**
 * Takes a complex type expression and simplifies it to a plain object. Useful when inferring types.
 */
export type Simplify<T> = T extends infer _ ? {
    [K in keyof T]: T[K];
} : never;
