(function(global, factory) {
	typeof exports === "object" && typeof module !== "undefined" ? factory(exports) : typeof define === "function" && define.amd ? define(["exports"], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, factory(global.StoryblokJSClient = {}));
	...
	*/
	const stringify = (params, prefix, isArray) => {
		...
		for (const key in params) {
			...
			let pair;
			if (typeof value === "object") pair = stringify(value, prefix ? `${prefix}[${enkey}]` : enkey, Array.isArray(value));
			else pair = `${prefix ? `${prefix}[${enkey}]` : enkey}=${encodeURIComponent(value)}`;
			pairs.push(pair);
		}
		return pairs.join("&");
	};
	...
	*/
	...
	Object.values(StoryblokContentVersion);
	/**
	...
	*/
	function determineRateLimit(url, params, config = {}, defaultRateLimit) {
		...
		return getRateLimitTier(params.per_page || 25);
	}
	/**
	...
	*/
	function throttledQueue(fn, limit, interval) {
		const isUnlimited = !Number.isFinite(limit) || limit <= 0 || !Number.isFinite(interval) || interval <= 0;
		const starts = [];
		...
		const acquire = () => {
			...
			return new Promise((resolve, reject) => {
				const attempt = () => {
					...
					const now = Date.now();
					while (starts.length > 0 && starts[0] <= now - interval) starts.shift();
	...
	*/
	function normalizeExcludingStoryFields(value) {
		if (Array.isArray(value)) return value.length > 0 ? value.join(",") : void 0;
		return value;
	}
	var Storyblok = class {
		client;
		...
		*/
		...
		parseParams(params) {
			...
			if (normalizedExcluding !== void 0) params.excluding_story_fields = normalizedExcluding;
			else delete params.excluding_story_fields;
		...
		get(slug, params = {}, fetchOptions) {
			...
			else if (params.version) delete params.version;
		...
		post(slug, params = {}, fetchOptions) {
			const url = `/${slug}`;
			const rateLimit = determineRateLimit(void 0, void 0, this.rateLimitConfig, 3);
		...
		*/
		...
		async cacheResponse(url, params, retries, fetchOptions) {
			...
			const defaultLimit = !isCDNUrl(url) && this.rateLimitConfig.isManagementApi ? 3 : void 0;
				...
				try {
					...
					if (error.response && error.status === 429) {
						...
						if (retries < this.maxRetries) {
							...
							return this.cacheResponse(url, params, retries, fetchOptions).then(resolve).catch(reject);
						}
					}
					reject(error);
				}
