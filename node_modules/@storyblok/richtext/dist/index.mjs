...
*/
const EXCLUDED_ATTRS = /* @__PURE__ */ new Set([
	"level",
	"linktype",
	"uuid",
	"anchor",
...
*/
const DEFAULT_VALUE_ONE_ATTRS = /* @__PURE__ */ new Set([
	"colspan",
	"rowspan",
	"order"
]);
...
function processAttribute(key, value, type, styleMap, attrMap, style, rest) {
	...
	if (DEFAULT_VALUE_ONE_ATTRS.has(key) && value === 1) return;
	if (key in styleMap) {
...
//#endregion
//#region src/static/normalize-nodes.ts
function normalizeNodes(input, includeKeys = false) {
	if (!input) return [];
	if (Array.isArray(input)) {
		if (!includeKeys) return input;
		return addKeys(input, createKeyGenerator());
	}
	const nodes = input.type === "doc" ? input.content || [] : [input];
	if (!includeKeys) return nodes;
	return addKeys(nodes, createKeyGenerator());
}
function addKeys(nodes, generateKey) {
	return nodes.map((node) => {
		const withKey = {
			...node,
			_key: generateKey(node.type)
		};
		if ("marks" in node && Array.isArray(node.marks)) withKey.marks = node.marks.map((mark) => ({
			...mark,
			_key: generateKey(mark.type)
		}));
		if ("content" in node && Array.isArray(node.content)) withKey.content = addKeys(node.content, generateKey);
		return withKey;
	});
}
//#endregion
...
* ```
*/
function renderRichText(document, context) {
...
function nodeAttrs(node) {
	return "attrs" in node ? node.attrs : void 0;
}
/** Renders a single node to HTML. */
function renderNode(node, context) {
	const content = node.type !== "text" && "content" in node && node.content ? renderChildren(node.content, context) : "";
	...
	if (node.type === "text") return renderTextNode(node, node.marks, context);
	if (node.type === "blok") {
	...
	if (node.type === "image" && context?.optimizeImage) return renderOptimizedImage(node, context);
	const attrs = nodeAttrs(node);
	const htmlAttrs = buildHtmlAttrs(node.type, attrs);
	if (isSelfClosing(tag)) return `<${tag}${htmlAttrs}>`;
	if (node.type === "table") return `<${tag}${htmlAttrs}>${renderTableRows(node.content, context)}</${tag}>`;
	const staticChildren = getStaticChildren(node);
	if (staticChildren) return `<${tag}>${renderStaticStructure(node.type, staticChildren, attrs, content)}</${tag}>`;
	if (node.type === "emoji") return `<${tag}${htmlAttrs}>${escapeHtml(("attrs" in node ? node.attrs?.emoji : void 0) ?? "")}</${tag}>`;
	return `<${tag}${htmlAttrs}>${content}</${tag}>`;
}
/** Renders an image node with optimization applied. */
function renderOptimizedImage(node, context) {
	const attrs = "attrs" in node ? node.attrs : void 0;
	const src = attrs?.src;
...
function renderTextNode(node, marks, context) {
	let html = escapeHtml(node.text);
	if (!marks?.length) return html;
	for (const mark of marks) html = wrapWithMark(html, mark, context);
	return html;
}
/** Wraps content with a single mark tag. */
function wrapWithMark(content, mark, context) {
	...
	if (!tag) return content;
	const markAttrs = "attrs" in mark ? mark.attrs : void 0;
	return `<${tag}${buildHtmlAttrs(mark.type, markAttrs)}>${content}</${tag}>`;
}
/** Link Mark Merging */
/** Renders consecutive text nodes (from start to end) under a single link tag. */
function renderLinkGroup(children, start, end, linkMark, context) {
	let inner = "";
...
function renderTableRows(rows, context) {
	if (!rows?.length) return "";
	let headerEnd = 0;
	while (headerEnd < rows.length && isTableHeaderRow(rows[headerEnd])) headerEnd++;
	let result = "";
	if (headerEnd > 0) {
		result += "<thead>";
		for (let i = 0; i < headerEnd; i++) result += renderNode(rows[i], context);
		result += "</thead>";
	}
	if (headerEnd < rows.length) {
		result += "<tbody>";
		for (let i = headerEnd; i < rows.length; i++) result += renderNode(rows[i], context);
		result += "</tbody>";
	}
	return result;
}
...
//#endregion
export { areLinkMarksEqual, attrsToHtmlString, optimizeImage as buildStoryblokImage, getInnerMarks, getStaticChildren, getTextNodeLinkMark, groupLinkNodes, hasContent, isSelfClosing, isTableHeaderRow, normalizeNodes, processAttrs, renderRichText, resolveTag, splitTableRows, stringToStyle, styleToString };
